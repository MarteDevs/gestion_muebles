 ...
 // Método para la página de inicio
    ...
    public String filtrarProductos(
        ...
        if (precio != null && !precio.isEmpty()) {
            ...
            try {
                if (p.endsWith("+")) {
                    String base = p.substring(0, p.length() - 1).trim();
                    minPrecio = Double.parseDouble(base);
                } else if (p.contains("-")) {
                    String[] rango = p.split("-");
                    if (rango.length > 0 && !rango[0].isBlank()) {
                        minPrecio = Double.parseDouble(rango[0].trim());
                    }
                    if (rango.length > 1 && !rango[1].isBlank()) {
                        maxPrecio = Double.parseDouble(rango[1].trim());
                    }
                } else {
                    minPrecio = Double.parseDouble(p);
                }
            } catch (NumberFormatException ex) {
                minPrecio = 0;
                maxPrecio = Double.MAX_VALUE;
            }
            final double min = minPrecio;
            final double max = maxPrecio;
